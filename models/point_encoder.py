...
class PointcloudEncoder(nn.Module):
    def __init__(self, point_transformer):
        ...
        self.trans_dim = 1024 # 768 如果换不同的模型，这里可能要改
        ...
        self.patch_dropout = nn.Identity() #PatchDropout(args.patch_dropout) if args.patch_dropout > 0. else
        self.visual = point_transformer


    def forward(self, pts, colors,layer=-1):
        # divide the point cloud in the same form. This is important
        _, center, features = self.group_divider(pts, colors)   # center: b,512,3   features: b,512,64,6

        # encoder the input cloud patches
        group_input_tokens = self.encoder(features)  #  B G N
        group_input_tokens = self.encoder2trans(group_input_tokens)     # B,G,768
        # prepare cls
        cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1)      # B,1,768
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)       # B,1,768
        # add pos embedding
        pos = self.pos_embed(center)    # B,G,768
        # final input
        x = torch.cat((cls_tokens, group_input_tokens), dim=1)  # B,G+1,768
        pos = torch.cat((cls_pos, pos), dim=1)      # B,G+1,768
        # transformer
        x = x + pos     # B,G+1,768
        # x = x.half()
        
        # a patch_dropout of 0. would mean it is disabled and this function would do nothing but return what was passed in
        x = self.patch_dropout(x)   # B,G+1,768

        x = self.visual.pos_drop(x) # B,G+1,768
        ...
        for i, blk in enumerate(self.visual.blocks):
            ...
            if i == layer:
                return x,center
        x = self.visual.norm(x[:, 0, :])    # 只用class token那个位置的作为输出,是分类
        x = self.visual.fc_norm(x)

        x = self.trans2embed(x)     # B,1024
        return x