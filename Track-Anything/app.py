...
def get_frames_from_video(video_input, video_state):
    ...
    try:
        ...
        while cap.isOpened():
            ret, frame = cap.read() # ret: true  frame:480,640,3
            if ret == True:
    ...
    image_size = (frames[0].shape[0],frames[0].shape[1])    # 480,640
...
def get_frames_from_custom(video_input,video_state):
    video_path = video_input
    frames = []
    user_name = time.time()
    operation_log = [("",""),("Upload video already. Try click the image for adding targets to track and inpaint.","Normal")]
    try:
        frame = cv2.cvtColor(cv2.imread(video_path), cv2.COLOR_BGR2RGB)
        fps = 30.0
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    # masks, logits, painted_images = model.generator(images=following_frames, template_mask=template_mask)   # todo 原track方法
    masks, logits, painted_images = model.generator_pose_prediction(template_mask=template_mask)
    # clear GPU memory
    model.xmem.clear_memory()
...
def generate_video_from_frames(frames, output_path, fps=30):
    ...
    # for frame in frames:
    #     video.write(frame)
    output_path = './result/track/gripper.mp4'
    # video.release()
...
with gr.Blocks() as iface:
    ...
    extract_frames_button.click(
        fn=get_frames_from_custom, # todo 做了改动 get_frames_from_video,
        inputs=[
