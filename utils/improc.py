...
class Summ_writer(object):
    ...
    def summ_boxlist2d(self, name, rgb, boxlist, scores=None, tids=None, frame_id=None, frame_str=None, only_return=False, linewidth=2):
        B, C, H, W = list(rgb.shape)
    ...
    def summ_traj2ds_on_rgbs(self, name, trajs, rgbs, valids=None, frame_ids=None, frame_strs=None, only_return=False, show_dots=True, cmap='coolwarm', vals=None, linewidth=1):
        # trajs is B, S, N, 2
        # rgbs is B, S, C, H, W
    ...
    def summ_traj_as_crops(self, name, trajs_e, rgbs, frame_id=None, frame_str=None, only_return=False, show_circ=False, trajs_g=None, is_g=False):
    ...
    def summ_pts_on_rgb(self, name, trajs, rgb, valids=None, frame_id=None, frame_str=None, only_return=False, show_dots=True, cmap='coolwarm', linewidth=1):
        # trajs is B, S, N, 2
        # rgbs is B, S, C, H, W
        B, C, H, W = rgb.shape
        B, S, N, D = trajs.shape
...
def dilate2d(im, times=1, mode='square'):
    device = im.device
    weights2d = torch.ones(1, 1, 3, 3, device=device, dtype=im.dtype)
    if mode=='cross':
        weights2d[:,:,0,0] = 0.0
        weights2d[:,:,0,2] = 0.0
        weights2d[:,:,2,0] = 0.0
